...
# 'Bank Customer Churn Dataset' from Kaggle https://www.kaggle.com/datasets/gauravtopre/bank-customer-churn-dataset

# This script can be used to create a Logistic Regression model using the input data only,
# or alternatively used to extend the model with engineered features.
# To run the model with engineered feature uncomment the following lines:
# - 75 -> 111
# - 129 -> 140
...
## Comment out the rest of Section 3 to remove the engineered features from the model
...
## Comment out the lines 134 -> 145 to remove engineered features
...
plt.plot(fpr, tpr, linewidth=3, label=f"ROC AUC = {test_roc_auc:.3f}", color='Mediumseagreen')
...
plt.plot(recall, precision, linewidth=3, color='Mediumseagreen')
...
prob_data = pd.DataFrame({
    ...
    # 'Actual': ['No Churn' if x != 1 else 'Churn' for x in y_test]
})
sns.histplot(data=prob_data, x='Probability', hue='Actual', bins=50,
             # palette={'No Churn': '#228B22', 'Churn': '#8B0000'},
             palette={'Churn': 'Red', 'No Churn': 'Mediumseagreen'},
             alpha=0.7, kde=True, legend=True)
...
legend_elements = [
    Patch(facecolor='Green', alpha=0.6, label='No Churn'),
    Patch(facecolor='Red', alpha=0.6, label='Churn'),
...
# 9.7. Model Performance Metrics Comparison
...
ax = sns.barplot(x='Metric', y='Score', data=metrics_df,
                 palette='Greens', hue='Metric', legend=False)
...
# 9.8. Churn Rate by Age
plt.figure(figsize=(10, 7))
age_churn = df.groupby('age')['churn'].mean()
sns.lineplot(x=age_churn.index, y=age_churn.values, linewidth=3, color='Mediumseagreen', marker='o')
...
# 9.9. Churn Rate by Number of Products (with dual axis)
fig, ax1 = plt.subplots(figsize=(10, 7))
product_churn = df.groupby('products_number')['churn'].agg(['mean', 'count'])

# Bar plot for churn rate
color1 = 'Mediumseagreen'
ax1.bar(product_churn.index, product_churn['mean'], color=color1, alpha=1.0, label='Churn Rate')
...
# Force integer x-axis values only (no decimals)
ax1.set_xticks(product_churn.index)  # Set ticks to actual product numbers only
ax1.xaxis.set_major_locator(plt.MaxNLocator(integer=True))  # Force integers

# Line plot for customer count on secondary y-axis
ax2 = ax1.twinx()
color2 = 'Royalblue'
ax2.plot(product_churn.index, product_churn['count'], 'o-', linewidth=3,
         markersize=10, color=color2, label='Customer Count')
...
# 9.10. Churn Rate by Country
plt.figure(figsize=(10, 6))
country_churn = df.groupby('country')['churn'].agg(['mean', 'count']).reset_index()
ax = sns.barplot(x='country', y='mean', data=country_churn, palette='viridis', hue='country', legend=False)
plt.ylabel('Churn Rate', fontsize=12)
...
# 9.12. Correlation Heatmap of Key Features
