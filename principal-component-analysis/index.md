...
* **Anomaly Detection in PCA Space** — The PCA-transformed space provides a natural framework for identifying outliers and borderline cases. New observations that project far from both cluster centroids — or into the overlap zone between malignant and benign — could be automatically flagged for secondary review. This positions PCA not just as a preprocessing step, but as a component of a clinical decision support workflow where model uncertainty is made explicit.
