...
A linear SVM fitted to this data achieves a test accuracy of [INSERT]%. The decision boundary plot below demonstrates the failure directly — the linear boundary bisects the feature space with a straight line, inevitably misclassifying observations from both classes that fall on the wrong side of the divide.

Show Image

An RBF kernel SVM fitted to the same data achieves a test accuracy of [INSERT]%. Rather than imposing a linear constraint, the RBF kernel implicitly maps observations into a higher-dimensional space in which the two concentric classes become separable, and the resulting decision boundary curves correctly around the inner class. The accuracy improvement of [INSERT] **percentage points** over the linear SVM is a direct, quantified demonstration of the practical value of the kernel trick.
