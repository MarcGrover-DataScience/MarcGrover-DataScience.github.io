...
The feature importance analysis reinforces a coherent chemical narrative. Alcohol content, volatile acidity, and sulphates emerge as the principal drivers of the model's classification decisions — findings that align with the established understanding of wine chemistry and validate that the model is learning genuine domain-relevant relationships. This interpretability is a meaningful strength: the model's behaviour can be explained in terms meaningful to a winemaker or quality manager, not only to a data scientist.

The Low class — representing wines with scores of 3 or 4 — remains difficult to classify reliably due to its small absolute count in the dataset. This is a data limitation that no modelling choice can fully compensate for, and per-class F1-scores provide a more complete view of performance across the quality spectrum than the overall accuracy figure alone. In any applied deployment of this model, particularly in a quality control context where the identification of poor-quality wines carries operational significance, per-class recall for the Low band should be the primary performance criterion rather than aggregate accuracy.
