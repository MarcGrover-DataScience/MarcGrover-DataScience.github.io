...
The difference between the observed post-intervention sales and this counterfactual is the estimated causal effect of the promotion. Both a point estimate and a 95% Bayesian credible interval are produced for every day in the post-period, along with cumulative and average effect summaries.  Unlike a traditional p-value, the Bayesian credible interval provides a direct probability statement, i.e. that there is a 95% probability that the true average daily effect lies between the lower effect value and the upper effect value.  If this interval excludes zero, it is considered strong evidence that the promotion had a genuine causal impact on sales, rather than explained by random variation.
...
**Store 30 Daily Sales Time Series with Intervention Marker** - This chart plots Store 30's daily sales across the full 2013–2014 window, with a vertical marker indicating the 3rd March 2014 intervention date when the Promo2 continuous promotion was activated. The chart provides a visual baseline for the causal analysis, allowing the reader to observe the pre-intervention sales pattern and form an initial impression of whether sales behaviour appears to shift following the intervention.
...
Several enhancements would strengthen the analysis and provide richer insight. Extending the post-period into 2015, would increase the statistical power of the analysis and provide a longer window over which to observe whether the cumulative effect stabilises, reverses, or continues to decline.  A longer post-period may also be sufficient to produce a statistically significant result, resolving the current ambiguity around the p-value.  
