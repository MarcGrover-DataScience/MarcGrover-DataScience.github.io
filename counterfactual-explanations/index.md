...
The Borderline Case is visibly absent from the chart above, and this is worth explaining rather than leaving as an unexplained gap. The sweep direction for each case is chosen from its predicted class: a case predicted `<=50K` is searched upward toward higher `capital_gain`, while a case predicted `>50K` is searched downward toward zero. The Borderline Case is predicted `>50K`, but its actual `capital_gain` is already 0 — so the search sweeps downward from 0 to 0, producing 400 grid points that are all identical. Rather than a curve spanning the axis, this collapses to a single point stacked on itself at `capital_gain = 0`, which is visually indistinguishable from nothing at all. This is not a plotting error: it is the correct, faithful output of a search that genuinely has no room to move in the one direction it was asked to explore, and it is itself a small but meaningful result — this case's position on the decision boundary has nothing to do with capital activity in the first place, which is exactly why the Regime A/B comparison above shows it as the one case where locking `capital_gain`/`capital_loss` makes no practical difference.
...
- **The model's performance and its narrow reliance on two features are not in tension, and this project's results do not support a diagnosis of underfitting or excessive generality.** A model that has failed to learn the data's structure typically shows weak discrimination throughout; this one instead achieves a strong 0.9141 test ROC-AUC (from the MLP project) alongside a highly specific, high-magnitude reliance on `capital_gain`/`capital_loss`. That combination is closer to the model having identified the single strongest signal in the data than to it having learned nothing general — the EDA project already showed that non-zero capital activity, while rare (present in only 8.3%/4.7% of records for `capital_gain`/`capital_loss` respectively), is overwhelmingly associated with `>50K` income where it does occur, so a strong, narrow reliance on it is a defensible reflection of a genuine, if unevenly distributed, pattern in the data — not evidence the network failed to find one.

- **The more precise concern this project's evidence supports is narrow reliance at the expense of interaction structure, not generality.** The False Positive case — 19 years old, 6 years of education, 24 hours/week, but a `capital_gain` of 34,095 — is predicted `>50K` with near-total confidence (0.999) despite an otherwise atypical profile for that outcome, and Regime B was unable to flip it using any combination of the remaining actionable features. A model that had learned to weigh `capital_gain` in the context of the rest of an individual's profile might discount it somewhat when the surrounding features look unusual; this model does not appear to do so. This is plausibly as much a consequence of data sparsity as of model capacity — training examples combining "non-zero capital gain" with an otherwise atypical profile are rare by construction, leaving the network little contradicting evidence from which to learn that nuance — but it is a genuine, demonstrated brittleness worth naming plainly rather than folding into the broader "the model relies on two features" finding.
