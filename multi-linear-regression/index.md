...
The model accounts for 47.7% of variance in tip values on the test set (R² = 0.4772), with a mean absolute error of 0.67. Two complementary model specifications underpin this analysis: the scikit-learn LinearRegression model, fitted on scaled features, provides the standardised coefficients used for feature importance comparison and generates the train/test predictions; the statsmodels OLS model, fitted on unscaled features, provides the inferential layer — adjusted R², p-values, t-statistics, and confidence intervals — which scikit-learn does not produce. The adjusted R² of 0.4612 from the OLS model, which penalises for the number of predictors, is the more appropriate summary statistic for a multi-predictor regression and provides a marginally more conservative estimate of fit than the unadjusted figure.
...
**16 observations exert disproportionate model influence**, introducing measurable uncertainty into reported performance metrics. Cook's Distance identifies 16 observations above the 4/n threshold — 6.6% of the dataset — predominantly corresponding to unusually high tip percentages relative to bill value. With a test set of only 49 observations, the random allocation of these influential observations between training and test sets has a material effect on results: a higher concentration in the training set pulls the fitted coefficients toward extreme tipping behaviour, while a higher concentration in the test set makes the reported test metrics less representative of true generalisation performance. This is a likely contributor to the high standard deviation of R² score (±0.2012) observed across cross-validation folds — when an influential observation falls in a given test fold it exerts a disproportionate effect on that fold's R² score. This reinforces the importance of the cross-validated mean (0.4159) over the single-split result as the primary performance reference, and identifies outlier treatment — whether through robust regression, removal with justification, or separate modelling — as a priority in any follow-on analysis.
