...
The omnibus chi-squared test confirmed that the variants do not perform equivalently, but the actionable insight comes from the Bonferroni-corrected pairwise comparisons. Only the A vs C comparison — a 10 percentage point gap — is statistically significant after correction. Neither A vs B nor B vs C reaches the adjusted threshold of α = 0.0167, meaning the data does not confirm that adjacent variants are meaningfully different from one another. Without multiple testing correction, all three pairwise comparisons would appear significant at α = 0.05, inflating the combined false positive probability to approximately 14% — illustrating precisely why the correction is not optional in multi-variant designs.

The practical recommendation is to deploy Variant C: it is the only variant with a statistically confirmed advantage over another and carries the highest observed open rate at 25%. If the distinction between B and C is commercially important, a dedicated two-variant follow-up test — with sample size informed by a power calculation for a 5 percentage point effect — would provide the necessary resolution.

### Example 3: Mobile App Onboarding

The new onboarding flow delivers a statistically significant improvement in 7-day retention, with a 10 percentage point increase from 30% to 40% and a Cramér's V indicating a medium effect — a meaningfully stronger practical magnitude than the small effect observed in Example 1. The recommendation to deploy the new onboarding flow is clear and well-supported by the evidence.

The downstream implications of a retention improvement of this scale are considerable. Users retained beyond the first week are more likely to become long-term active users, generate recurring revenue, and provide richer behavioural data for future product decisions. The 7-day retention window is a standard early-stage metric, but it is a starting point rather than a complete picture: confirming whether the improvement persists at 14 and 30 days would help distinguish genuine product adoption from short-term re-engagement, and would strengthen the business case further.
